...
public class QuantizationStream extends FilterInputStream {
    ...
    public QuantizationStream(IepeInputStream voltage, int sampleSizeInBits, double rate) {
        super(voltage);
        this.sampleSizeInBits = sampleSizeInBits;
        this.rate = rate;
    }

    public QuantizationStream(IepeInputStream voltage, int sampleSizeInBits) {
    ...
    public byte[] readQuantization() throws IOException {
        ...
        max >>= 1;
        long data = (long) (((IepeInputStream) in).readValue() * rate * max);
        for (int i = 0; i < buffer.length; i++) {
